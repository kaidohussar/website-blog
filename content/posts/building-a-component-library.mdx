...
### Implementing design system into a UI library
...
After trying and investigating multiple technologies I personally think the 2 best options on building a UI library supported by design system is either using <b>[styled components](https://styled-components.com/)</b>
...
```
- **TypeScript support in writing CSS** - benefits of being CSS in JS/TS, as the main variables are coming from the theme object, it will provide TS support.
- **Customizable and dynamic** - can easily overwrite theme object and extend ui parts. In the same way that you can access your theme, as shown above, you can also access any
...
Cons:
* **Learning curve** - Often considered a myth, but in my opinion as with any other framework or tool, bringing it into a team and forcing best practices might not be that easy when components are really complex.
* **Separations of concern** - Even tough there are ways to differentiate logic and styles, in many cases these 2 worlds still mix. With CSS it is really easy to understand what code changes logic, what styles.
...
Pros:
- **Separation of logic and styles** - As styles and other logic is really separated, it makes HTML alot more clear.
...
- **Multiple themes with different styles** - as styled components is really flexible because of the nature of writing JS in CSS, it's a lot more convinent to apply different styles depending on theme.
...
- **Longevity in mind** - CSS is here to stay and will be here for years to come.
...
### Tools for building and maintaining your UI library

#### Bit.dev
It's a platform that manages all your reusable components. It makes it easy to version components separately and update the dependencies. Testing, documentation, versioning all handled by 1 platform.
...
#### What I would use?
If I were to start a large scale project and would need a great out-of-the-box solution, I would definitely go with **bit.dev**. Going with modular and maintaining packages as separate entities is way to go.

For constructing a UI library on a documentation and display purposes I would choose [Storybook](https://storybook.js.org/), as there is a lot of tools and addons available for different needs. For more customizable option I would choose any of the static docs generators.
